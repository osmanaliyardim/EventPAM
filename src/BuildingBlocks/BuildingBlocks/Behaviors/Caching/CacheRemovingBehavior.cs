    ...
    where TRequest : IRequest<TResponse>, ICacheRemoverRequest
{
    ...
    public async Task<TResponse> Handle(TRequest request, RequestHandlerDelegate<TResponse> next, CancellationToken cancellationToken)
    {
        ...
        if (request.CacheGroupKey != null)
        {
            ...
            if (cachedGroup != null)
            {
                ...
                foreach (string key in keysInGroup)
                {
                    ...
                    _logger.LogInformation($"{Messages.CACHE_REMOVED}{key}");
                }

                await _cache.RemoveAsync(request.CacheGroupKey, cancellationToken);
                _logger.LogInformation($"{Messages.CACHE_REMOVED}{request.CacheGroupKey}");
            }
        }
        ...
        if (request.CacheKey != null)
        {
            ...
            _logger.LogInformation($"{Messages.CACHE_REMOVED}{request.CacheKey}");
        }
