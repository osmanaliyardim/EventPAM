...
namespace EventPAM.BuildingBlocks.ElasticSearch;
...
public class ElasticSearchManager : IElasticSearch
{
    ...
    {
        ElasticSearchConfig? settings = configuration.GetSection(Configs.ELASTIC_CONFIG).Get<ElasticSearchConfig>();
    ...
    public async Task<IElasticSearchResult> CreateNewIndexAsync(IndexModel indexModel)
    {
        ...
        if (elasticClient.Indices.Exists(indexModel.IndexName).Exists)
            return new ElasticSearchResult(success: false, message: Messages.ES_INDEX_ALREADY_EXISTS);
    ...
    private ElasticClient GetElasticClient(string indexName)
    {
        if (string.IsNullOrEmpty(indexName))
            throw new ArgumentNullException(indexName, message: Messages.ES_INDEX_NULL);
